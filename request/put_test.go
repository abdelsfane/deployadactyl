...
var _ = Describe("PutDeploymentRequest", func() {
	Describe("GetId", func() {
		It("should return the UUID", func() {
			request := PutDeploymentRequest{
				Request: PutRequest{
					UUID: "abcd123",
				},
			}
	...
	Describe("SetContext", func() {
		It("should return a new modified PutDeploymentRequest", func() {
			origRequest := PutDeploymentRequest{
				Deployment: interfaces.Deployment{
					CFContext: interfaces.CFContext{
						Environment: "the environment",
					},
	...
	Describe("SetRequest", func() {
		It("should return a new modified PutDeploymentRequest", func() {
			...
			Expect(request.GetRequest().(PutRequest).UUID).To(Equal("UUID test data"))
			Expect(newRequest.GetRequest()).To(Equal(expected))
		})

		Context("if provided object is not a PutRequest", func() {
