...
- name: healthy-timmy
  ...
  - route: custom-route-2.example.com`
	...
	BeforeEach(func() {
		...
		provider = creator.CreatorModuleProvider{
			...
			NewCourier: func(executor interfaces.Executor) interfaces.Courier {
				...
				courier.DomainsCall.Returns.Domains = []string{"notValid.com"}
	...
	It("returns failed route mapper error", func() {
		Expect(string(responseBody)).To(ContainSubstring("invalid route provided, check that the domain exists in the foundation"))
	})
})
