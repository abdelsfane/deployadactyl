...
`
	...
	BeforeEach(func() {
		...
		provider = creator.CreatorModuleProvider{
			...
			NewCourier: func(executor interfaces.Executor) interfaces.Courier {
				...
				if len(couriers) == 2 {
					courier.StopCall.Returns.Error = errors.New("failed to start")
				}
	...
	It("calls courier stop with correct info", func() {
		for _, c := range couriers {
			Expect(c.StopCall.Received.AppName).To(ContainSubstring(appName))
		}
	})
	It("calls courier start with correct info", func() {
		for _, c := range couriers {
	...
	It("emits a StopStartedEvent", func() {
		Expect(reflect.TypeOf(eventManager.EmitEventCall.Received.Events[0])).To(Equal(reflect.TypeOf(stop.StopStartedEvent{})))
	})
	It("emits a StopFailureEvent", func() {
		Expect(reflect.TypeOf(eventManager.EmitEventCall.Received.Events[1])).To(Equal(reflect.TypeOf(stop.StopFailureEvent{})))
	})
	It("emits a StopFinishedEvent", func() {
