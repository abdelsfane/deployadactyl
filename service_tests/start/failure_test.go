...
`
	...
	BeforeEach(func() {
		...
		provider = creator.CreatorModuleProvider{
			...
			NewCourier: func(executor interfaces.Executor) interfaces.Courier {
				...
				if len(couriers) == 2 {
					courier.StartCall.Returns.Error = errors.New("failed to start")
				}
	...
	It("emits a StartFailureEvent", func() {
		Expect(reflect.TypeOf(eventManager.EmitEventCall.Received.Events[1])).To(Equal(reflect.TypeOf(start.StartFailureEvent{})))
	})
